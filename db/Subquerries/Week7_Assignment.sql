...
-- Using a correlated subquery that compares each instructor's salary to the average salary of instructors in every OTHER department (excluding their own) , How many such instructors are there
Select Count(i1.instructor_id) From Instructor i1 Where salary > (Select Avg(Salary) From Instructor i2 Where i2.dept_id <> i1.dept_id);


-- Select Students with no enrollment request
SELECT name FROM Student
WHERE student_id IN (SELECT student_id FROM EnrollmentRequest);
...
-- How many instructors earn more than every instructor in the Mechanical department (dept_id = 3)
Select i1.name from Instructor i1 Where salary > ALL(SELECT i2.salary FROM Instructor i2 WHERE dept_id = 3);
